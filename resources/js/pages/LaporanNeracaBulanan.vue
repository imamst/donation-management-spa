            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex flex-row justify-content-between">
                        ...
                        <a type="button" :href="`/api/laporan/neraca/export?bulan=${month}`" class="btn btn-sm btn-success float-right"><i class="fas fa-file-excel"></i> Export Excel</a>
                    </div>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                        ...
                        <table id="pengeluaranTable" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <tr v-for="(neraca, index) in neraca_list" :key="neraca.id">
                                    <td>{{ index+1 }}</td>
                                    <td>
                                        {{ dayjs(neraca.tanggal_batch).format('DD MMMM YYYY') }}
                                    </td>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getNeracaList(month = this.month) {
                Neraca.getMonthlyNeraca(month)
                    .then(response => {
