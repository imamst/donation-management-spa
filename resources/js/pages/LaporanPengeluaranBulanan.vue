            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex flex-row justify-content-between">
                        ...
                        <a type="button" :href="`/api/laporan/pengeluaran/export?bulan=${month}`" class="btn btn-sm btn-success float-right"><i class="fas fa-file-excel"></i> Export Excel</a>
                    </div>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                        ...
                        <table id="pengeluaranTable" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <tr v-for="(pengeluaran, index) in pengeluaran_list" :key="pengeluaran.id">
                                    <td>{{ index+1 }}</td>
                                    <td>
                                        <button type="button" class="btn btn-link text-primary" @click.prevent="showPengeluaranHarian(pengeluaran.tanggal_batch)">{{ dayjs(pengeluaran.tanggal_batch).format('DD MMMM YYYY') }}</button>
                                    </td>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getPengeluaranList(bulan = this.month) {
                Pengeluaran.getMonthlySpending(bulan)
                    .then(response => {
            ...
            showPengeluaranHarian(tanggal) {
                this.$router.push(`/laporan/pengeluaran/harian/${tanggal}`)
            },
